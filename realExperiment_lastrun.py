...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.79.01), Do 25 Jun 2015 13:46:46 CEST
...
text = visual.TextStim(win=win, ori=0, name='text',
    text='Now the real experiment starts.\nThe changing dots will be presented in 6 trials that each last 1.5 minutes.\nAfter each trial a short break is placed. Feel free to stand up or drink in that time, and as soon as you feel ready Press \'Space\' to continue.\n\nPress "Space" to continue\n\n',    font='Arial',
...
# Initialize components for Routine "HorzVert"
HorzVertClock = core.Clock()
rating = visual.RatingScale(win=win, name='rating', escapeKeys=['escape'], displaySizeFactor=1.00, pos=[0.0, -0.4], choices=[u'Horizontal', u'Vertical'])
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        elif cubeLU.status == STARTED and t >= (0.0 + 0.4):
        ...
        elif cubeRD.status == STARTED and t >= (0.0 + 0.4):
        ...
        elif cubeLD.status == STARTED and t >= (0.4 + 0.4):
        ...
        elif image.status == STARTED and t >= (0.0 + 0.8):
    ...
    for thisTimeGetterTrial in timeGetterTrial:
        ...
        for thisRepetitionsOfDistanceCheck in repetitionsOfDistanceCheck:
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                elif blockRU.status == STARTED and t >= (0.4 + 0.4):
        ...
        #------Prepare to start Routine "HorzVert"-------
        t = 0
        HorzVertClock.reset()  # clock 
        frameN = -1
        # update component parameters for each repeat
        rating.reset()
        ...
        # keep track of which components have finished
        HorzVertComponents = []
        HorzVertComponents.append(rating)
        for thisComponent in HorzVertComponents:
            if hasattr(thisComponent, 'status'):
                thisComponent.status = NOT_STARTED
        
        #-------Start Routine "HorzVert"-------
        continueRoutine = True
        while continueRoutine:
            # get current time
            t = HorzVertClock.getTime()
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            # *rating* updates
            if t > 0.0:
            ...
            save = "./TIMES/%s" % expInfo['participant']
            if rating.noResponse == False:
                with open(save, "a") as myfile:
                    String.append(rating.getRating())
                    String.append("\n")
        ...
        #-------Ending Routine "HorzVert"-------
        for thisComponent in HorzVertComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        timeGetterTrial.addData('rating.response', rating.getRating())
        timeGetterTrial.addData('rating.rt', rating.getRT())
        myTime = 0
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    elif text_2.status == STARTED and t >= (0.0 + 5.0):
